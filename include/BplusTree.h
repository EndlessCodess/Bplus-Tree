...
private:
  ...
  //删除后调整操作
  bool adjust(std::shared_ptr<LeafNode<keyType, valueType>> node,
              std::shared_ptr<InterNode<keyType, valueType>> parent);

  //得到左兄弟
  std::shared_ptr<LeafNode<keyType, valueType>>
  getLeftSibling(std::shared_ptr<LeafNode<keyType, valueType>> node);

  //得到右兄弟
  std::shared_ptr<LeafNode<keyType, valueType>>
  getRightSibling(std::shared_ptr<LeafNode<keyType, valueType>> node);

  //从左兄弟借
  void borrowFromL(std::shared_ptr<LeafNode<keyType, valueType>> node,
                   std::shared_ptr<LeafNode<keyType, valueType>> leftSibling,
                   std::shared_ptr<InterNode<keyType, valueType>> parent);

  //从右兄弟借
  void borrowFromR(std::shared_ptr<LeafNode<keyType, valueType>> node,
                   std::shared_ptr<LeafNode<keyType, valueType>> rightSibling,
                   std::shared_ptr<InterNode<keyType, valueType>> parent);

  //与左兄弟合并
  void mergeWithL(std::shared_ptr<LeafNode<keyType, valueType>> node,
                  std::shared_ptr<LeafNode<keyType, valueType>> leftSibling,
                  std::shared_ptr<InterNode<keyType, valueType>> parent);

  //与右兄弟合并
  void mergeWithR(std::shared_ptr<LeafNode<keyType, valueType>> node,
                  std::shared_ptr<LeafNode<keyType, valueType>> rightSibling,
                  std::shared_ptr<InterNode<keyType, valueType>> parent);
