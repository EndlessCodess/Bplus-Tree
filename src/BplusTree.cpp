...
BplusTree<keyType, valueType>::findLeaf(
    std::shared_ptr<Node<keyType, valueType>> currentNode, const keyType &key) const
{
    ...
    std::shared_ptr<LeafNode<keyType, valueType>> leafNode)
{
  ...
  // 将新节点插入父节点
  std::shared_ptr<InterNode<keyType, valueType>> parent = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(leafNode->parent);
    ...
    std::shared_ptr<InterNode<keyType, valueType>> interNode)
{
  ...
  // 更新父指针结构
  std::shared_ptr<InterNode<keyType, valueType>> parent = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(interNode->parent);
    ...
    std::shared_ptr<Node<keyType, valueType>> node)
{
  ...
  if (node->parent)
  {
    auto parent = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(node->parent);
    ...
    std::shared_ptr<InterNode<keyType, valueType>> parent)
{
  ...
  { // 内部节点
    ...
    if (childIt != parent->children.end())
    {
      ...
      {
        currentRight = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(currentRight->children.front());
      }
    ...
    std::shared_ptr<InterNode<keyType, valueType>> parent)
{
  ...
  { // 内部节点
    ...
    if (childIt != parent->children.end())
    {
      ...
      {
        currentNode = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(currentNode->children.front());
      }
    ...
    std::shared_ptr<InterNode<keyType, valueType>> currentNode)
{
  ...
  auto parent = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(currentNode->parent);
  if (parent)
  { // 不为根结点
    adjust(std::dynamic_pointer_cast<Node<keyType, valueType>>(currentNode), parent);
  }
}
...
bool BplusTree<keyType, valueType>::remove(const keyType &key)
{
  ...
  {
    auto parent = std::dynamic_pointer_cast<InterNode<keyType, valueType>>(targetLeaf->parent);
    if (parent)
    { // 非根节点
      return adjust(std::dynamic_pointer_cast<Node<keyType, valueType>>(targetLeaf), parent);
    }
    else
    { // 根结点
  ...
  if (targetLeaf->keys.size() > maxKeys)
  {
    ...
    while (currentNode)
    {
      ...
      if (currentNode->keys.size() > maxKeys && currentNode->isLeafNode())
      {
        ...
        splitLeaf(std::dynamic_pointer_cast<LeafNode<keyType, valueType>>(currentNode));
      }
      // 内部节点分裂
               ...
               !currentNode->isLeafNode())
      {
        ...
        splitInter(std::dynamic_pointer_cast<InterNode<keyType, valueType>>(currentNode));
      }
      else
      {
        break;
...
// 实例化(int->uint_64)
template class BplusTree<int, uint64_t>;